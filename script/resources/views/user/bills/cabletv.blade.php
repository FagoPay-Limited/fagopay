...
<div class="ccontainer-fluid  ">
  <div class="content-wrapper card-body">
      <form action="{{ route('user.cabletvbuy') }}" method="post" class="ajaxform_instant_reload">
                      @csrf
    <div class="row ">
      <div class="col-md-8"><div class="card"><div class="card-body">
          
         
        <h6 class="h2 d-inline-block mb-0">{{__('Cable Tv')}}</h6>
                         ...
                         <div class="text-center">
                                   <center>
                                        <div class=" form-check avatar mb-2">
                                            <input class="type form-check-input v-image__image v-image__placeholder" style="background-image: url({{url('/')}}/asset/images/{{$data->image}});background-size: contain;" value="{{$data->code}}"  onChange="populate()" id="{{$data->code}}" name="decoder" type="radio"/>
        ...
        <div class="form-group col-md-6">
                                                    <label class="text-muted">Select Bundle</label>
        ...
        <div class="form-group"><label class="text-muted">IUC Number</label>
          <input class="form-control iuc" name="iuc"  id="iuc" type="number" placeholder="IUC Number">
        </div>
...
if($('input[value=dstv]').prop('checked')) { 
$('.select').html(' <select id="s2" name="plan" class="form-control form-control-solid "><option selected disabled>Select Dstv Plan</option>@foreach($bill as $data)@if($data["networkcode"] == "dstv")  <option class="ss2" rel="bundle" type="radio" nam="{{$data["name"]}}" cost="{{$data["cost"]}}" value="{{$data["plan"]}}" >{{$data["name"]}}</option>@endif @endforeach</select>');
        
}

if($('input[value=gotv]').prop('checked')) { 
$('.select').html('<select id="s2" name="plan" class="form-control form-control-solid "><option selected disabled>Select Gotv Plan</option>@foreach($bill as $data)@if($data["networkcode"] == "gotv")<option class="ss2" rel="bundle" type="radio" nam="{{$data["name"]}}" cost="{{$data["cost"]}}" value="{{$data["plan"]}}">{{$data["name"]}}</option>@endif @endforeach</select>');
        
}

if($('input[value=startimes]').prop('checked')) { 
$('.select').html('<select id="s2" name="plan" class="form-control form-control-solid "><option selected disabled>Select Startimes Plan</option>@foreach($bill as $data)@if($data["networkcode"] == "startimes")<option class="ss2" rel="bundle" type="radio" nam="{{$data["name"]}}" cost="{{$data["cost"]}}" value="{{$data["plan"]}}" >{{$data["name"]}}</option>@endif @endforeach</select>');
...
$(function() {
  $('.iuc').on("input", function() {
    $('#tot').text($('#s2').find('option:selected').attr('cost'));
    $('#dis').val($('#s2').find('option:selected').attr('cost'));
    $('#amt2').text($('#s2').find('option:selected').attr('nam'));
...
$(function() {
     ...
     $('#tot').text($(this).find('option:selected').attr('cost'));
      $('#dis').val($(this).find('option:selected').attr('cost'));
     $('#amt2').text($(this).find('option:selected').attr('nam'));
    });
});
</script>
            ...
            if (email.length > 1) {
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        let $tt = $('#s2').find('option:selected').attr('cost');
