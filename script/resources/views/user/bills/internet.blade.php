
@extends('layouts.user.master')

@section('content')


<div class="ccontainer-fluid ">
  <div class="content-wrapper card-body">
      
      <form action="{{ route('user.internetbuy') }}" method="post" class="ajaxform_instant_reload">
                      @csrf
                      
                      
    <div class="row align-items-center ">
      <div class="col-md-8"><div class="card"><div class="card-body">
          
        <h6 class="h2 d-inline-block mb-0">{{__('Buy Data')}}</h6>
      
       
          <div class="bg-white border-0 mb-0">
                   <br>
                  <div class="">
                  
                      
                        
                        <div class="col-12 row container">
                        <p data-v-62dda23e="" class="text-title font-weight-bold">Select provider</p>
                                                  <div class="col-3">
                         <div class="text-center">
                                   <center>
                                        <div class=" form-check avatar mb-2">
                                            <input class="form-check-input v-image__image v-image__placeholder" style="background-image: url({{url('/')}}/asset/images/airtel.png);background-size: contain;" id="airtel" value="airtel" name="network" type="radio" onChange="populate()">
                                            <label for="contact3" class="avatar shadow-sm">
                                                
                                            </label>
                                        </div>
                                        <p class="text-color-theme size-12 small">Airtel</p>
</center>
                                    </div></div>
                                                             <div class="col-3">
                         <div class="text-center">
                                   <center>
                                        <div class=" form-check avatar mb-2">
                                            <input class="form-check-input v-image__image v-image__placeholder" style="background-image: url({{url('/')}}/asset/images/glo.png);background-size: contain;" id="glo" value="glo" name="network" type="radio" onChange="populate()">
                                            <label for="contact3" class="avatar shadow-sm">
                                                
                                            </label>
                                        </div>
                                        <p class="text-color-theme size-12 small">Glo</p>
</center>
                                    </div></div>
                                                             <div class="col-3">
                         <div class="text-center">
                                   <center>
                                        <div class=" form-check avatar mb-2">
                                            <input class="form-check-input v-image__image v-image__placeholder" style="background-image: url({{url('/')}}/asset/images/mtn.png);background-size: contain;" id="mtn" value="mtn" name="network" type="radio" onChange="populate()">
                                            <label for="contact3" class="avatar shadow-sm">
                                                
                                            </label>
                                        </div>
                                        <p class="text-color-theme size-12 small">MTN</p>
</center>
                                    </div></div>
                                                             <div class="col-3">
                         <div class="text-center">
                                   <center>
                                        <div class=" form-check avatar mb-2">
                                            <input class="form-check-input v-image__image v-image__placeholder" style="background-image: url({{url('/')}}/asset/images/9mobile.png);background-size: contain;" id="etisalat" value="etisalat" name="network" type="radio" onChange="populate()">
                                            <label for="contact3" class="avatar shadow-sm">
                                                
                                            </label>
                                        </div>
                                        <p class="text-color-theme size-12 small">9mobile</p>
</center>
                                    </div></div>
                                                        
                            </div>
                        
                    
                            
                      
                      <div class="row">
                          
                            <div class="col-md-12">
        <div class="form--group"><label>Select Bundle</label>
          <div class="input-group mb-3">
                                     
                                     <div data-v-7d4385f0="" class=" mt-sm-4 no-gutters" style="max-height: 300px; overflow-y: scroll;">

                                     <div id="s2" class="row select" style="    margin-left: 0px;"></div>
                                     
                                    </div>

                                                    </div>
        </div></div>
                               
                                                    
                      
                                <div class="form-group "> 
                                <br>
                           <select name="type"  id="myselection" class="form-control"> 
                                <option>Select Airtime Beneficiary</option>
                                <option value="new">New Number</option>
                                <option value="red">Saved Beneficiary</option>
                                
                            </select>

                            </div>
                            
                       
        
        
        
        
                                                    
                            <div class=" form-group row red box">
                          
                            <div class="input-group">
                              <span class="input-group-prepend">
                                <span class="input-group-text"><i class="fa fa-users"></i></span>
                              </span>
                            <select id="bene" name="choosebeneficiary" class="form-control" data-placeholder="Beneficiary">
                              <option label="Choose one">Select Beneficiary
                              </option>
                              @foreach($benefit as $data)
                               <option value="{{$data->is_beneficiary}}">{{$data->is_beneficiary_name}} ( {{$data->is_beneficiary}} )</option>
                              @endforeach
                            </select>
                            </div>
                        </div>
                            <div class=" form-group row new box">
                                
                            <div class="input-group">
                              <span class="input-group-prepend">
                                <span class="input-group-text">+234</span>
                              </span>
                              <input type="number" class="call form-control" name="phone" placeholder="Phone Number" minlength="11" ><br>
                            </div>
                            

                               
                          <div class="col-md-6">
                          <div class="custom-control custom-control-alternative custom-checkbox"><br>
                                                                                            <input type="checkbox" name="saveben" id="customCheckLogin8" class="custom-control-input">
                                                                                        <label class="custom-control-label" for="customCheckLogin8">
                                            <span class="text-muted">Save New Beneficiary</span>     
                                            </label>
                                        </div>  
                          </div>
                         <div class="col-lg-6 yeah2" style="display:none;">
                                           <br>
                                            <input type="text" name="beneficiary" class="form-control" placeholder="Save As?">
                                        </div>
                                        
                                        </div>
                                </div>
                       
                            
                            
                                                
                                                
                            <input id="dis" name="dis" type="hidden" >
                            <div id="yeah" class="yeah form-group">
                           <br>
                          <div class="col-md-12">
                            <div class="input-group" id="preview">
                                
                              <span class="input-group-prepend">
                                <span id="email2" class="input-group-text">Discount code</span>
                              </span>
                              <input type="text" id="email" class="form-control @error('email') is-invalid @enderror" name="email" placeholder="">
                            </div>
                            
                                
                          </div>
                         
                              


                        </div>
                        
                         <br>
        
                  </div>
                </div>
            
            
      </div></div></div>
      
    
       <div class="col-md-4">
         
          <div class="card">
            <div class="card-body">
          <div data-v-fde0cb54="" class="modal-instructions mx-auto"><div data-v-fde0cb54="" class="v-item-group theme--light v-expansion-panels v-expansion-panels--flat v-expansion-panels--focusable"><div data-v-fde0cb54="" aria-expanded="true" class="v-expansion-panel v-expansion-panel--active v-item--active">
              
              <button data-v-fde0cb54="" type="button" class="v-expansion-panel-header v-expansion-panel-header--active" style="background-color: #480007;
    border-color: rgb(244, 245, 247);
    color: #fff;border-radius:10px">

              <h6 class="h2 d-inline-block mb-0" style="color: #fff;">{{__('Overview')}} 
        <svg data-v-1a72eceb="" width="16" height="17" viewBox="0 0 16 17" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" class="mt-1 ml-2"><rect data-v-1a72eceb="" y="0.5" width="16" height="16" fill="url(#pattern0)"></rect><defs data-v-1a72eceb=""><pattern data-v-1a72eceb="" id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1"><use data-v-1a72eceb="" xlink:href="#image0" transform="scale(0.00625)"></use></pattern><image data-v-1a72eceb="" id="image0" width="160" height="160" xlink:href="data:image/png;base64,"></image></defs>
        </svg>
        </h6>
            </button> 
            <div data-v-fde0cb54="" class="v-expansion-panel-content" style="">
                <div class="v-expansion-panel-content__wrap">
                    <div class="row">
                        <div class="col-6">
                           
                <p class="mt-4 mb-1">Phone No.</p> 
                </div>
                <div class="col-6">
                <p class="mt-4 mb-1 text-right"><strong class="airtime"></strong></p> 
                </div>
                </div>
                <div class="dividerr"></div>
                <div class="row">
                        <div class="col-6">
                <p class="mt-4 mb-1">You Pay</p> 
                </div>
                <div class="col-6">
                <p class="mt-4 mb-1 text-right">₦<strong id="amt2">0</strong></p> 
                </div>
                </div>
                <div class="dividerr"></div>
                
                 <div class="row">
                        <div class="col-6">
                <p class="mt-4 mb-1">Discount</p> 
                </div>
                <div class="col-6">
                <p class="mt-4 mb-1 text-right">₦<strong id="dis2">0</strong></p> 
                </div>
                </div>
                <div class="dividerr"></div>
                
                 <div class="row">
                        <div class="col-6">
                <p class="mt-4 mb-1">You receive</p> 
                </div>
                <div class="col-6">
                <p class="mt-4 mb-1 text-right">₦<strong id="tot">0</strong> worth of Data</p>
                </div>
                </div>
                
                <div class="dividerr"></div>
                
                </div></div></div> </div> </div>
                
                
                
                  <div class="text-center">
                            <button type="submit"  class="submit btn btn-neutral  submit-btn" >{{__('Buy Data Plan')}} <!--<span id="resulttransfer"></span>--></button>
                        </div> 
                   
                    </div>
                    
                    </div>
            
            
      
            
      </div>
      
      
      
    </div> </form></div></div>
    
    
    
    
    
    
<div class="content-wrapper card-body">
    <div class="card">
      <div class="card-header header-elements-inline">
        <h3 class="mb-0 font-weight-bolder">{{__('Transactions')}}</h3>
      </div>
    
        <div class="modal fade" id="single-charge" tabindex="-1" role="dialog" aria-labelledby="modal-form" aria-hidden="true">
          <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h3 class="mb-0 font-weight-bolder">{{__('Data Bundle')}}</h3>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                      <span aria-hidden="true">&times;</span>
                    </button>
                </div>
              <div class="modal-body">
                  
                  
                  
                  
              </div>
            </div>
          </div>
        </div>
    
    
    
<div data-v-be36719e="" class="v-data-table theme--light">
            <div class="v-data-table__wrapper">
      <div class="table-responsive py-4">
        <table class="table table-flush" id="datatable-buttons">
        <thead>
													<tr>
														<th>ID</th>
														<th>Phone</th>
														<th>Network</th>
														<th>Amount</th>
														<th>Bundle</th>
														<th>Status</th>
														<th>Date</th>
													</tr>
												</thead>
												<tbody>
												@foreach($bills as $data)
													<tr>
														<td>#{{$data->trx}}</td>
														<td class="text-success">{{$data->phone}}</td>
														<td>{{strtoupper($data->network)}}<br>
														</td>

														<td>₦{{number_format($data->amount,2)}}</td>
														<td>{{strtoupper($data->accountname)}}<br>
														</td>


                                                       @if($data->status == 0)
														<td><span class="badge bg-warning badge-pill">Pending</span></td>
														@elseif($data->status == 1)
														<td><span class="badge bg-success badge-pill">Completed</span></td>
														@else
														<td><span class="badge bg-danger badge-pill">Declined</span></td>
														@endif
														<td>{{date(' d M, Y ', strtotime($data->created_at))}} {{date('h:i A', strtotime($data->created_at))}}</td>
													</tr>
											  @endforeach


												</tbody>
											</table>
										
      </div>
    </div>
    
    </div>
    </div>
    </div>
    
@push('script')    

    
   
<script>

function populate(){
    
if($('input[value=glo]').prop('checked')) { 
$('.select').html('@foreach($bill as $data)@if($data["networkcode"] == "glo")<div data-v-6209a816="" data-v-7d4385f0="" xs="10" class="data-plan col-sm-2 col" style="padding: 0px;"><input id="{{$data["plan"]}}" nam="plan" rel="{{$data["cost"]}}" type="radio"  onChange="popu()" cost="{{$data["cost"]}}" value="{{$data["plan"]}}" class="ss2" name="plan" ><label for="{{$data["plan"]}}" class="data-plan__title mt-1">{{$data["name"]}}</label></div>@endif @endforeach');
        
}

if($('input[value=mtn]').prop('checked')) { 
$('.select').html('@foreach($bill as $data)@if($data["networkcode"] == "mtn")<div data-v-6209a816="" data-v-7d4385f0="" xs="10" class="data-plan col-sm-2 col" style="padding: 0px;"><input id="{{$data["plan"]}}" class="ss2" nam="plan" rel="bundle" type="radio" onChange="popu()" cost="{{$data["cost"]}}" value="{{$data["plan"]}}" name="plan" ><label for="{{$data["plan"]}}" class="data-plan__title mt-1">{{$data["name"]}}</label></div>@endif @endforeach')
   
}

if($('input[value=airtel]').prop('checked')) { 
$('.select').html('@foreach($bill as $data)@if($data["networkcode"] == "airtel")<div data-v-6209a816="" data-v-7d4385f0="" xs="10" class="data-plan col-sm-2 col" style="padding: 0px;"><input id="{{$data["plan"]}}" class="ss2" nam="plan" rel="bundle" type="radio" onChange="popu()" cost="{{$data["cost"]}}" value="{{$data["plan"]}}" name="plan" ><label for="{{$data["plan"]}}" class="data-plan__title mt-1">{{$data["name"]}}</label></div>@endif @endforeach')
   
}

if($('input[value=etisalat]').prop('checked')) { 
$('.select').html('@foreach($bill as $data)@if($data["networkcode"] == "9mobile")<div data-v-6209a816="" data-v-7d4385f0="" xs="10" class="data-plan col-sm-2 col" style="padding: 0px;"><input id="{{$data["plan"]}}" class="ss2" nam="plan" rel="bundle" type="radio" onChange="popu()" cost="{{$data["cost"]}}" value="{{$data["plan"]}}" name="plan" ><label for="{{$data["plan"]}}" class="data-plan__title mt-1">{{$data["name"]}}</label></div>@endif @endforeach')
   
}

$('#tot').text('');
$('.call').val('');
$('.airtime').text('');
$('#amt2').text('');
$('#dis').val('');
};

    
    
</script>



 <script type="text/javascript">


        
        var startTimer;
        $('#email').on('keyup', function () {
            clearTimeout(startTimer);
            let $checkDiv = $('#email2');
            $checkDiv.attr({'class':'input-group-text checking'});
            $checkDiv.text('checking...');
            let email = $(this).val();
            startTimer = setTimeout(checkEmail, 300, email);
            
        });

        $('#email').on('keydown', function () {
            clearTimeout(startTimer);
        });
        
        function checkEmail(email) {
            $('#email-error').remove();
            
            if (email.length > 1) {
                $.ajax({
                    type: 'post',
                    url: "{{ route('user.checkEmail') }}",
                    data: {
                        email: email,
                        _token: "{{ csrf_token() }}"
                    },
                    success: function(data) {
                        let $checkDiv2 = $('#email2');
                        let $tt = $('input[name="plan"]:checked').attr("cost")
                        let $cal = {{$discount->discount}}/100*$tt;
                        if (data.success == false) {
                            $checkDiv2.attr({'class':'input-group-text text-danger'});
                            $checkDiv2.text(''+data.message+'');
                            $('#dis').val($tt);
                            $('#dis2').text(0);
                            $('#amt2').text($tt);
                        } else {
                            $checkDiv2.attr({'class':'input-group-text text-success'});
                            $checkDiv2.text(''+data.message+'');
                            $('#dis').val($tt-$cal);
                            $('#dis2').text($cal);
                            $('#amt2').text($tt-$cal);
                        }

                    }
                });
            } 
            else {
                let $tt = $('input[name="plan"]:checked').attr("cost")
                $('#email2').attr({'class':'input-group-text'});
                $('#email2').text('Discount Code');
                $('#dis').val($tt);
                $('#dis2').text(0);
                $('#tot').text($tt);
            }
        }
  
</script>
<script>
$(function() {
  $('.call').on("input", function() {
    $('.airtime').text($(this).val());
    $('#tot').text($('input[name="plan"]:checked').attr("cost"));
    $('#dis').val($('input[name="plan"]:checked').attr("cost"));
    $('#amt2').text($('input[name="plan"]:checked').attr("cost"));
    $('.yeahh').show();
    
  });
});


$(function() {
  $('#bene').on("change", function() {
    $('.airtime').text($(this).val());
    $('#tot').text($('input[name="plan"]:checked').attr("cost"));
     $('#dis').val($('input[name="plan"]:checked').attr("cost"));
    $('#amt2').text($('input[name="plan"]:checked').attr("cost"));
    $('.yeahh').show();
    
  });
});


  
$(document).ready(function(){
    $("#myselection").change(function(){
        $(this).find("option:selected").each(function(){
            var optionValue = $(this).attr("value");
            if(optionValue){
                $(".box").not("." + optionValue).hide();
                $("." + optionValue).show();
            } else{
                $(".box").hide();
            }
        });
    }).change();

});
  
  
$('input[name=saveben]').change(function(){
  if(this.checked){
    $('.yeah2').show();
  }
  else{
    $('.yeah2').hide();
  }
});
  
  
  function popu(){
     $('.ss2').change(function () {
         if ($(this).prop('checked')) {
     $('#tot').text($('input[name="plan"]:checked').attr("cost"));
      $('#dis').val($('input[name="plan"]:checked').attr("cost"));
     $('#amt2').text($('input[name="plan"]:checked').attr("cost"));
    }
    else{
      $('#tot').text('');
      $('#amt2').text('');
      $('#dis').val('');
    }
     
})
    
};



</script>
@endpush
@endsection