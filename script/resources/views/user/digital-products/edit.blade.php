...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active" aria-current="page">{{ __('Digital Products Edit') }}</li>
@endsection

@section('actions')
<a href="{{ route('user.digital-products.index') }}" class="btn btn-sm btn-neutral"><i class="fa fa-eye" aria-hidden="true"></i> {{ __('View list') }}</a>
@endsection
...
@section('content')
    <div class="row justify-content-center">
        <div class="col-sm-8">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('user.digital-products.update', $product->id) }}" method="post" enctype="multipart/form-data" class="ajaxform_instant_reload">
                        @csrf
                        @method('put')
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-6 mb-4">
                                <label for="link">{{ __("Download Link") }}</label>
                                <input type="url" name="link" class="form-control" required="" value="{{ $product->link }}">
                            </div>
                            ...
                            <div class="col-sm-12 mb-4">
                                <label for="confirmation_message">{{ __("Confirm message") }}</label>
                                <textarea name="confirmation_message" id="confirmation_message" class="form-control" placeholder="{{ __("Confirmation message") }}">{{ $product->confirmation_message }}</textarea>
                            </div>
                        </div>
