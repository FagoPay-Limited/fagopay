...
@section('title', __('Physical Products'))

@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('user.home.index') }}"><i class="fas fa-home"></i></a></li>
    <li class="breadcrumb-item active" aria-current="page">{{ __('Physical products list') }}</li>
@endsection

@section('actions')
    <a href="{{ route('user.physical-products.create') }}" class="btn btn-sm btn-neutral"><i class="fa fa-plus" aria-hidden="true"></i> {{ __('Add physical products') }}</a>
@endsection
    ...
    <div class="col-md-12">
        ...
        <div class="card">
            <div class="card-header pb-2">
                <h4></h4>
                <form action="{{ route('user.physical-products.index') }}" class="card-header-form">
                    @csrf
                                ...
                                <td>
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu">
                                            <a class="dropdown-item" href="{{ route('user.physical-products.edit', $product->id) }}"><i class="fas fa-edit mr-1"></i>  {{ __('Edit') }}</a>

                                            <a class="dropdown-item confirm-action" href="#"
                                                data-action="{{ route('user.physical-products.destroy', $product->id) }}"
                                                data-method="DELETE"
...
<input type="hidden" id="get-products-url" value="{{ route('user.get-physical-products') }}">
@endsection
